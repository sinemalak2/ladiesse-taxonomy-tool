...
//
// Uses App A's client-credentials grant (lib/shopify.js) — la-diesse is a
// store WE own, already has this custom app installed, and client
// credentials needs no OAuth consent screen or Partners distribution
// method. The per-brand OAuth app (lib/brandShopifyAuth.js) is reserved for
// what it's actually needed for: pulling from each BRAND's own store, which
// we don't control and can't add a custom app to directly.
...
async function getLadiesseShopInfo() {
  const data = await shopifyGraphql(SHOP_INFO_QUERY);
  return {
...
async function importOneProduct({ product, variants, brandName, rate, locationId }) {
  ...
  const data = await shopifyGraphql(PRODUCT_SET_MUTATION, { input, identifier });
...
export async function importBrandProductsToLadiesse({ brandId, jobId = null }) {
  ...
  try {
    ...
    const { currency: ladiesseCurrency, locationId } = await getLadiesseShopInfo();
    ...
    for (const product of products) {
      ...
      const result = await importOneProduct({ product, variants, brandName, rate, locationId });
...
// Removes a brand's products from la-diesse.myshopify.com (the drafts
// created by importBrandProductsToLadiesse above) and clears the
// imported_shopify_*_gid tracking columns, so a future import creates fresh
// products instead of trying to update ones that no longer exist. Used only
// by the brand DELETE route (app/api/brands/[id]/route.js) as a cleanup step
// before the brand row (and the brand_products rows carrying these GIDs)
// disappears — NOT by the standalone "Delete products" admin action, which
// is DB-only (see deleteBrandProductsFromDb below) so staff can clear a
// brand's pulled catalogue for re-testing without touching the live store.
export async function deleteLadiesseImportedProducts({ brandId }) {
  const client = await getPool().connect();
  try {
    const { rows: products } = await client.query(
      `SELECT id, imported_shopify_product_gid FROM brand_products
       WHERE brand_id = $1 AND imported_shopify_product_gid IS NOT NULL`,
      [brandId]
    );
    ...
    for (const product of products) {
      try {
        const data = await shopifyGraphql(PRODUCT_DELETE_MUTATION, { input: { id: product.imported_shopify_product_gid } });
        const userErrors = data.productDelete.userErrors;
        if (userErrors.length > 0) {
          throw new Error(userErrors.map((e) => e.message).join('; '));
        }
        await client.query(
          `UPDATE brand_products SET imported_shopify_product_gid = NULL, imported_at = NULL WHERE id = $1`,
          [product.id]
        );
        await client.query(
          `UPDATE brand_product_variants SET imported_shopify_variant_gid = NULL WHERE brand_product_id = $1`,
          [product.id]
        );
        deleted++;
      } catch (err) {
        errors.push({ brandProductId: product.id, message: err.message });
      }
    }

    return { deleted, total: products.length, errors };
  } finally {
    client.release();
  }
}

// Clears a brand's pulled catalogue from our own DB only — never touches
// la-diesse.myshopify.com or the brand's own store. Lets staff wipe stale
// test/synced data (e.g. rows pulled before a filter change, or from a
// throwaway dev-store brand) and re-run "Sync now" cleanly, without any
// risk of deleting something live. brand_product_variants cascades via FK.
export async function deleteBrandProductsFromDb({ brandId }) {
  const { rowCount } = await getPool().query('DELETE FROM brand_products WHERE brand_id = $1', [brandId]);
