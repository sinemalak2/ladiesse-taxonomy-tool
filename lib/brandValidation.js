...
// US Employer Identification Number: 9 digits, conventionally displayed as
// XX-XXXXXXX. Unlike SSN or the Turkish VKN/TCKN, the IRS doesn't publish a
// checksum algorithm for EINs — digit-count/format is the standard level of
// validation available outside an actual IRS lookup.
export function validateEIN(value) {
  const s = String(value ?? '').trim().replace(/-/g, '');
  return /^\d{9}$/.test(s);
}

// US ABA routing number: 9 digits with a standard weighted checksum
// (weights 3,7,1 repeated across the three 3-digit groups), used to catch
// typos the same way the VKN/TCKN checks do.
...
// US bank account numbers have no universal checksum (unlike IBAN/routing
// numbers) — NACHA just bounds them to 4-17 digits.
export function validateUSAccountNumber(value) {
  const s = String(value ?? '').trim();
  return /^\d{4,17}$/.test(s);
}
...
// Drives which legal ID (VKN/TCKN vs EIN) and bank fields (IBAN vs
// routing/account) apply to a brand — see makeStep4Schema/makeStep6Schema.
export const COUNTRY_OPTIONS = ['TR', 'US'];
...
export const step3Schema = z.object({
  ...
  title: z.string().trim().min(2, 'Required, e.g. "Founder & CEO"').max(100),
  phone_number: z
    .string()
    .trim()
...
// tax_id's validation (and whether tax_office is required) depends on which
// country the brand is in — set at Step 2 and passed in here rather than
// resubmitted, since it's the same brand-level field either way.
export function makeStep4Schema(country) {
  const base = {
  ...
  if (country === 'US') {
    return z.object({
      ...base,
      tax_id: z.string().trim().refine(validateEIN, 'Invalid EIN — must be 9 digits, e.g. 12-3456789'),
...
// IBAN (TR) vs routing/account number (US) — same country flag as step 4.
export function makeStep6Schema(country) {
  const base = {
  ...
  if (country === 'US') {
    return z.object({
      ...
      routing_number: z
        .string()
        .trim()
        .refine(validateRoutingNumber, 'Invalid routing number (checksum failed)'),
      account_number: z
        .string()
        .trim()
        .refine(validateUSAccountNumber, 'Invalid account number (4-17 digits)'),
    });
  }
...
// Steps 4 and 6 are intentionally absent — their schema depends on the
// brand's country, built per-request via makeStep4Schema/makeStep6Schema.
